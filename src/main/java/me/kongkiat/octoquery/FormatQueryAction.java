...
public class FormatQueryAction extends AnAction {
    ...
    private static boolean isNativeQuery(PsiAnnotation annotation) {
        ...
        boolean isNativeAnnotation =
                qName.endsWith(".NativeQuery") || qName.equals("org.springframework.data.jpa.repository.NativeQuery");
    ...
    private static String formatQuery(Project project, String query, boolean isNative) {
        if (isNative) {
            return formatWithSqlFormatter(project, query);
        } else {
            return formatWithDtoAwareFormatter(project, query);
        }
    }
